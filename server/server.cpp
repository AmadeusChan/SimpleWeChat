...
class MainControl {
	...
	public: 
		...
		static std::string processReceiveFileRequest(json request) {
			...
			std::cout<<"processReceiveFileRequest:"<<request["receiver"]<<" "<<len<<std::endl;
			for (int i=0; i<len; ++i) {
		...
		static std::string sendNextFilePacket(json res_) { // assume that num<max_num
			int num, max_num;
			std::string file_name, sender, receiver, payload;
			num = res_["num"];
			num++;
			max_num = res_["max_num"];
			file_name = res_["file_name"];
			sender = res_["sender"];
			receiver = res_["receiver"];
			payload = getFilePacketPayload(num, max_num, file_name, sender, receiver);
